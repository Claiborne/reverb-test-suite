...
describe "USER FLOWS - Read an Article and Personalize", :read_article => true do
  ...
  it 'should send an article read event' do
    RestClient.post @event_url, read_article(Time.now.utc.to_i-160, @article_data[:article_id]), 'Content-Type' => 'application/json'
  end

  it 'should exit an article after 15 seconds' do
    sleep 15
    RestClient.post @event_url, exit_article(Time.now.utc.to_i-160), 'Content-Type' => 'application/json' 
  end
