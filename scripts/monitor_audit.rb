...
Collection\t|User Collections\t|Collection Recs\t|Interest Stream\t|Interest Search".yellow
...
1000.times do  
  ...
  interest_search = begin; data['com.wordnik.bifrost.apis.InterestsApi']['/interests/search']['duration']['median'].to_s.match(/^\d{0,}/).to_s; rescue; 0; end
  ...
  puts "#{news_wordwall}\t\t|#{news_tiles}\t\t|#{me_wordwall}\t\t|#{me_tiles}\t\t|#{article}\t\t|#{article_recs}\t\t|".green+
       ...
       "|#{interest_stream}\t\t\t|#{interest_search}".green
